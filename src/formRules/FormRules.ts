...
import { SetRuleOptions } from "@/utils/core/engine/useEngineManager"; 

// 定义一个 NM 别名方便阅读
type NM = Exclude<FormFieldSchema, GroupField>;
...
SetStrategy: (path: AllPath, key: KeysOfUnion<NM>, strategy: DefaultStrategy) => void,
notifyAll: () => void
){
    ...
    const calculatePrice = (api: any) => {
        ...
        const BasePrice =
          baseObj[compliance.value];
        const SpecsPrice =
          specsObj[instanceFamily.value];
        
        const RegionFactor =
          region.value === "china" ? 1 : 1.5;
      
        const unitPrice =
          diskPriceMap[diskType.value];
        const encryptionFactor = encryption.value === "yes" ? 1.1 : 1.0;
        
        let StorageCost = capacity.value * unitPrice * encryptionFactor;
      ...
      SetRule(
        ...
        "value", //副作用影响的键值对
        {
          logic: (api) => {
            ...
            return value.value === "financial" ? "hdd" : undefined;
          },
          triggerKeys:['value']
        }
      );
      ...
      SetRules(
        ...
        "options",
        {
          logic: (api) => {
            ...
            if (region.value === "china" && compliance.value === "standard") {
          ...
          effect:(args)=>{
            ...
            return isLegal?undefined:{
              value:options[0]?options[0].value:undefined
            }
          },
      ...
      SetStrategy("cloudConsole.billing.autoRenew", "disabled", DefaultStrategy.OR);
      ...
      SetRules(
        ...
        "description",
        {
          logic: async (api) => {
            ...
            if(compliance.value==='financial') return "金融级合规要求人工确认订单"
