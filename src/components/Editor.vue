...
import { FormFieldSchema, GroupField } from "@/utils/forms/schema/schema";

// import { useEngine } from "@meshflow/form";
import { useEngine } from "@/utils/forms/useMeshForm";
...
const engine = useEngine<
{
  ...
  useMeshRenderGate:typeof useMeshRenderGate
},
AllPath,
...
const schema = engine.modules.internalModules.internalForm.uiSchema;
 
const hasRenderGate = engine.config.hasRenderGate()
...
provide('notifyAll',engine.config.notifyAll);

provide("GetFormData",  engine.modules.internalModules.internalForm.GetFormData);
...
provide(
  'redoAction',
  engine.modules.history.Redo
  // ()=>{}
  );
 
 
provide('historyStatus',{
  updateUndoSize:engine.modules.history.updateUndoSize,
  updateRedoSize:engine.modules.history.updateRedoSize
  // updateUndoSize:()=>1,
  // updateRedoSize:()=>1
});

provide('onDirty',engine.modules.render.onDirty)
