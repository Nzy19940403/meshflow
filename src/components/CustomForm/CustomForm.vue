<template>
  ...
  ></v-skeleton-loader>
  <v-form ref="formRef" v-else style="min-height: 80vh;">
    <CustomFormNode
...
import { ref, onMounted, shallowRef, inject,nextTick } from "vue";

import CustomFormNode from "./CustomFormNode.vue";
import { RenderSchema } from "@/utils/forms/schema/schema";
...
const historyStatus = inject<{
    updateUndoSize:(cb:(value:number)=>void)=>void ,
    updateRedoSize:(cb:(value:number)=>void)=>void
}>('historyStatus') ;
...
const notifyAll = inject('notifyAll') as Function;
...
historyStatus!.updateUndoSize((value:number)=>{
    undoSize.value = value;
});
historyStatus!.updateRedoSize((value:number)=>{
