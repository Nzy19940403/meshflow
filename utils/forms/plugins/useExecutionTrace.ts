...
import { MeshEvents,MeshFlowEventsName } from "@meshflow/core";
 

type NodeStatus = 'idle' | 'pending' | 'calculating' | 'calculated' | 'error' | 'canceled';

export function useExecutionTrace<P>(

) {
  // 存储当前路径的状态快照
  const statusMap = new Map<P, NodeStatus>();

  // 存储每个路径对应的 UI 更新回调
  const callbackMap = new Map<P, (status: NodeStatus) => void>();

  // 保留：记录当前版图，用于重置和熔断时的遍历
  let currentSessionAffected = new Set<P>();

  /**
   * ⚡️ 核心：精准状态更新器 (替代 dispatch)
   * 只触发指定 path 的回调，其他人不打扰
   */
  const updateStatus = (path: P, newStatus: NodeStatus) => {
    // 防抖：状态没变就不触发
  ...
  const useTrace = ()=>{
    const apply = (api: { 
      ...
      // 4. 计算成功：完成施工
      api.on( MeshFlowEventsName.NodeSuccess , ({ path }: { path: P }) => {
